...
import { dailyHistoryDeck, historyLevel, historyPoints, historyQuestions, historyQuestionTarget } from '@/lib/history';
  ...
  return <main className="min-h-screen bg-paper text-ink">
    ...
    <section className="mx-auto max-w-xl px-5 py-12 md:py-16">
      ...
      <article className="mt-14 border-t border-ink pt-7">
        ...
        <div className="mt-12"><div className="mb-5 flex items-end justify-between"><span className="font-mono text-[9px] text-ink/40">{item.min}</span><strong className="text-4xl font-medium tracking-[-.04em]">{guess}</strong><span className="font-mono text-[9px] text-ink/40">{item.max}</span></div><input aria-label={`Choose a year from ${item.min} to ${item.max}`} className="timeline-slider w-full" disabled={submitted} max={item.max} min={item.min} onChange={(event) => setGuess(Number(event.target.value))} type="range" value={guess} /></div>
        {!submitted ? <button className="mt-10 w-full bg-ink px-6 py-4 text-sm font-medium text-paper" onClick={submit}>Check {guess}</button> : <div aria-live="polite" className="mt-10 border-t border-ink/20 pt-7">
          <div className="flex items-baseline justify-between gap-4"><div><p className="font-mono text-[9px] uppercase tracking-[.14em] text-ink/45">Answer · {item.year}</p><p className="mt-2 text-3xl font-medium tracking-[-.04em]">{distance === 0 ? 'Exact' : `${distance} years away`}</p></div><span className="font-mono text-sm">{points} pts</span></div>
