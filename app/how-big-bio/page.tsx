...
const questions = [
  { prompt: 'Protein molecules in one E. coli cell?', value: 3.5e6, unit: 'proteins', source: 'BioNumbers BNID 115702', sourceUrl: 'https://bionumbers.hms.harvard.edu/bionumber.aspx?id=115702', note: 'Uses a 3–4 million estimate for a roughly 1 µm³ E. coli cell; growth conditions and cell volume move the answer.' },
  { prompt: 'Base pairs in one copy of the human genome?', value: 3e9, unit: 'base pairs', source: 'NHGRI — Base Pair', sourceUrl: 'https://www.genome.gov/genetics-glossary/Base-Pair', note: 'Counts one haploid copy. Most human cells are diploid and therefore carry roughly two copies.' },
  { prompt: 'Neurons in an adult human brain?', value: 8.6e10, unit: 'neurons', source: 'Azevedo et al. 2009 table', sourceUrl: 'https://bionumbers.hms.harvard.edu/files/Expected%20values%20for%20a%20generic%20rodent%20and%20primate%20brains%20of%201.5%20kg%2C%20and%20values%20observed%20for%20the%20human%20brain.pdf', note: 'Uses the isotropic-fractionator estimate of 86 billion; this is a population-scale estimate, not an exact individual count.' },
  { prompt: 'Human cells in a 70 kg adult man?', value: 3e13, unit: 'human cells', source: 'BioNumbers BNID 113006', sourceUrl: 'https://bionumbers.hms.harvard.edu/bionumber.aspx?id=113006', note: 'Uses the 2016 revised estimate for a reference 70 kg adult man. About 70% of the counted cells are red blood cells.' },
  { prompt: 'Bacterial cells in that same reference body?', value: 3.8e13, unit: 'bacterial cells', source: 'BioNumbers BNID 113006', sourceUrl: 'https://bionumbers.hms.harvard.edu/bionumber.aspx?id=113006', note: 'The same analysis estimates 3.8×10¹³ bacteria—near 1:1 with human cells, replacing the old 10:1 rule of thumb.' },
] as const;

const superscript = (exponent: number) => String(exponent).split('').map((char) => '⁰¹²³⁴⁵⁶⁷⁸⁹'[Number(char)]).join('');
function scientific(value: number) {
  const exponent = Math.floor(Math.log10(value));
  const coefficient = value / 10 ** exponent;
  return `${coefficient.toFixed(coefficient < 2 ? 1 : 0)} × 10${superscript(exponent)}`;
}

export default function HowBigBio() {
  const [index, setIndex] = useState(0);
  const [guessExponent, setGuessExponent] = useState(9);
  const [locked, setLocked] = useState(false);
  const [total, setTotal] = useState(0);
  const item = questions[index];
  const error = Math.abs(guessExponent - Math.log10(item.value));
  const points = useMemo(() => Math.round(1000 * 0.5 ** error), [error]);
  const factor = 10 ** error;
  const guess = 10 ** guessExponent;
  const lock = () => { setLocked(true); setTotal((current) => current + points); };
  const next = () => { setIndex((current) => (current + 1) % questions.length); setGuessExponent(9); setLocked(false); };

  return <main className="min-h-screen bg-paper text-ink">
    <header className="border-b-2 border-ink"><div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-5 md:px-10"><a className="inline-flex items-center gap-2 font-mono text-[10px] font-bold uppercase" href="/games"><ArrowLeft className="h-4 w-4" /> AI Games</a><span className="font-mono text-[9px] font-bold uppercase text-cobalt">HOW BIG? / Bio Numbers</span></div></header>
    ...
    <section className="mx-auto grid max-w-6xl gap-10 px-5 py-12 md:px-10 lg:grid-cols-[.62fr_1.38fr]">
      ...
      <article className="border-2 border-ink bg-white p-6 shadow-[9px_9px_0_#d8ff39] md:p-9">
        ...
        <div className="mt-12"><div className="mb-4 flex items-end justify-between gap-4"><span className="font-mono text-[9px] text-ink/40">1</span><div className="text-center"><p className="font-mono text-[8px] font-bold uppercase text-coral">Your estimate</p><strong className="mt-1 block text-3xl">10<sup>{guessExponent.toFixed(1)}</sup></strong><span className="font-mono text-[8px] text-ink/45">≈ {guess.toExponential(1)}</span></div><span className="font-mono text-[9px] text-ink/40">10²⁷</span></div><input aria-label="Choose an exponent from zero to twenty-seven" className="timeline-slider w-full" disabled={locked} max="27" min="0" onChange={(event) => setGuessExponent(Number(event.target.value))} step="0.1" type="range" value={guessExponent} /><div className="mt-3 flex justify-between font-mono text-[7px] font-bold uppercase text-ink/35"><span>Cellular</span><span>Organism</span><span>Planetary-ish</span></div></div>
        {!locked ? <button className="mt-9 w-full bg-ink px-6 py-4 font-mono text-[10px] font-bold uppercase text-paper" onClick={lock}>Lock in 10^{guessExponent.toFixed(1)}</button> : <div className="mt-9 border-2 border-ink bg-paper p-5"><p className="font-mono text-[9px] font-bold uppercase text-coral">Reference answer</p><p className="mt-2 text-4xl font-black">{scientific(item.value)} {item.unit}</p><p className="mt-2 font-mono text-[9px] font-bold uppercase text-cobalt">{factor < 1.06 ? 'Bullseye' : `${factor.toFixed(factor < 10 ? 1 : 0)}× ${guess > item.value ? 'too high' : 'too low'}`} · {points} points</p><p className="mt-5 text-sm leading-relaxed text-ink/60">{item.note}</p><div className="mt-5 flex flex-wrap items-center justify-between gap-4"><a className="inline-flex items-center gap-1 font-mono text-[8px] font-bold uppercase text-cobalt underline" href={item.sourceUrl} target="_blank" rel="noreferrer">{item.source} <ExternalLink className="h-3 w-3" /></a><button className="inline-flex items-center gap-2 font-mono text-[9px] font-bold uppercase text-cobalt" onClick={next}>Next quantity <RotateCcw className="h-4 w-4" /></button></div></div>}
      </article>
    </section>
  </main>;
}
