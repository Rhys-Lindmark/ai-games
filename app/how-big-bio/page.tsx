...
import { ArrowLeft, ExternalLink, RotateCcw, Share2, Sigma } from 'lucide-react';
import { useMemo, useState } from 'react';
import { bioNumbersDeck, dailyBioDeck, magnitudeFactor, magnitudeScore, scientific, shareReceipt } from '@/lib/how-big';
...
export default function HowBigBio() {
  ...
  return <main className="min-h-screen bg-paper text-ink">
    ...
    <section className="mx-auto grid max-w-6xl gap-10 px-5 py-12 md:px-10 lg:grid-cols-[.62fr_1.38fr]">
      <aside><p className="font-mono text-[9px] font-bold uppercase text-coral">Daily five · reviewed pool of twelve</p><h2 className="mt-3 text-4xl font-black tracking-[-.05em]">BUILD A FEEL FOR ORDERS OF MAGNITUDE.</h2><p className="mt-5 text-sm leading-relaxed text-ink/60">Half the points survive each order of magnitude you miss by. Every reveal names the counted thing, reference population, and source.</p><div className="mt-7 border-2 border-ink bg-white p-4 font-mono text-[9px] font-bold uppercase shadow-[4px_4px_0_#1c1c1a]"><p>{dayKey} · Round {index + 1} / {questions.length}</p><p className="mt-2 text-cobalt">Score {total}</p><p className="mt-2 text-ink/45">Sources checked 2026-08-30</p></div></aside>
